...
from .TableauHyperApiExtraLogic import ClassBN as ClsBN
...
class CommandLineArgumentsHandling:
    ...
    def fn_assess_option(current_option, str_meaning, given_default_value):
        if ClsBN.cfg_dtls['options'][current_option]['default_value'] == given_default_value:
            str_feedback_parts = {
                'prefix': 'Fatal Error',
                'verdict': 'Expected -' + current_option + '|--'
                           + ClsBN.cfg_dtls['options'][current_option]['option_long']
                           + ' <'
                           + ClsBN.cfg_dtls['options'][current_option]['option_sample_value']
                           + '> but nothing of that sort has been seen...',
                'suffix': ':-('
            }
            ln = len(str_feedback_parts['verdict']) - len(str_feedback_parts['prefix'])
            ClsBN.fn_timestamped_print(ClsBN, str_feedback_parts['prefix'] + '_'*ln)
            print(str_feedback_parts['verdict'] + ' ' + str_feedback_parts['suffix'])
            sys.exit(2)
        ClsBN.fn_timestamped_print(ClsBN, str_meaning + ' is "' + given_default_value + '"')

    @staticmethod
    def fn_build_combined_options():
        str_combined_opts = ''
        for option_index, crt_opt in enumerate(ClsBN.cfg_dtls['options']):
            if crt_opt is not None:
                str_option_crt = '-' + crt_opt + '|--'\
                                 + ClsBN.cfg_dtls['options'][crt_opt]['option_long']
                if 'option_sample_value' in ClsBN.cfg_dtls['options'][crt_opt]:
                    str_option_crt += ' <'\
                                      + ClsBN.cfg_dtls['options'][crt_opt]['option_sample_value']\
                                      + '>'
                if ClsBN.cfg_dtls['options'][crt_opt]['option_type'] == 'optional':
                    str_combined_opts += ' [' + str_option_crt\
                                         + '|if omitted, default value will be considered: '\
                                         + str(ClsBN.cfg_dtls['options'][crt_opt]['default_value'])\
                                         + ']'
                else:
                    str_combined_opts += ' ' + str_option_crt
    ...
    def fn_build_long_options():
        str_opts = []
        for option_index, crt_lng_opt in enumerate(ClsBN.cfg_dtls['options']):
            if crt_lng_opt is not None:
                str_opts.append(option_index)
                str_opts[option_index] = ClsBN.cfg_dtls['options'][crt_lng_opt]['option_long'] + '='
        return str_opts

    @staticmethod
    def fn_build_short_options():
        str_short_options = 'h'
        for crt_short_opt in ClsBN.cfg_dtls['options']:
            if crt_short_opt is not None:
                if ClsBN.cfg_dtls['options'][crt_short_opt]['option_type'] == 'mandatory':
